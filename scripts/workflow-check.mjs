#!/usr/bin/env node
import { findSpec, statusFor, specStatus, milestones, loadConfig, currentBranch } from './workflow-lib.mjs';
...
if (command === 'ci') {
  ...
  let spec;
  try { spec = findSpec(specArg); }
  catch (e) {
    // Non-spec branches are allowed through this structural gate.
    console.log(`Workflow CI: no spec resolved for branch ${branch}; structural spec gate skipped.`);
    process.exit(0);
  }
  if (specStatus(spec.text) !== 'LOCKED') fail(`Workflow CI blocked: ${spec.id} is not LOCKED.`);
  if (milestones(spec.text).length === 0) fail(`Workflow CI blocked: ${spec.id} contains no implementation milestones.`);
  console.log(`Workflow CI structural gate passed for ${spec.id}.`);
  process.exit(0);
}

let spec;
try { spec = findSpec(specArg); } catch (e) {
  if (command === 'prepush' && !specArg) {
    const config = loadConfig();
    const branch = currentBranch();
    if (config.protectedBranches.includes(branch)) fail(`Direct push blocked for protected branch ${branch}. Use the repository PR/release workflow.`);
    console.log(`Feature workflow gate: no SPEC ID detected on branch ${branch || '(detached)'}; gate not applicable.`);
    process.exit(0);
  }
  fail(e.message);
}
...
if (command === 'status') {
  ...
  for (const m of state.milestones) console.log(`Milestone ${m.milestone} QA: ${m.pass ? 'PASS' : 'PENDING'}`);
...
if (command === 'prepush') {
  if (state.next !== 'READY_TO_PUSH') {
    fail([
      ...
      ...state.milestones.map(m => `Milestone ${m.milestone} QA: ${m.pass ? 'PASS' : 'PENDING'}`),
